...
optimal testing grounds for stellar models. Traditional observations
...
pulsations that do not vary significantly throughout their orbit. One such
...
for its light to reach Earth varies, which periodically modulates the pulsation phases. A map of the binary orbit can then be
...
to large variations when the stars are at their closest approach [@Murphy2016Finding]. Since the
...
effect directly onto the light curve, which allows for every data point in the light
curve to be modelled simultaneously, removing the need to choose a subdivision
