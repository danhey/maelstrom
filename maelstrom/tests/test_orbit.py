...
def test_orbit_basics():
    with pm.Model() as model:
        orbit = Orbit(period=pm.Flat('period', testval=100),
                     lighttime=pm.Flat('lighttime', testval=100),
                     eccen=pm.Flat('eccen', testval=0.),
                     omega=pm.Flat('omega', testval=0.),
                     phi=pm.Flat('phi', testval=0.),
                     freq=pm.Flat('freq', testval=np.array(1.)))
