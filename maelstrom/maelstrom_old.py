...
class BaseOrbitModel(Model):
    ...
    def corner_plot(self, trace, varnames=None):
        ...
        trace : pymc3 trace object
            Trace resulting from the sample() method
        varnames : list-like, optional
            List of parameter names to show in the corner plot. If none,
            will default to showing period, t0, varpi, eccen, and logsigma
        """
        try:
            import corner
        except:
...
class PB1Model(BaseOrbitModel):
    ...
    def init_params(self, period=None, eccen=None, with_eccen=True):
        pass

    def init_orbit(self, period=None, eccen=None, with_eccen=True, with_gp=True):
        with self:
            if period is None:
                period = self.get_period_estimate()
            ...
            logP = pm.Bound(pm.Normal,
                        lower=np.log(10),
                        upper=np.log(4000))("logP", mu=np.log(period), sd=1.0,
            ...
            if with_gp:
                ...
                pm.Potential("obs", gp.log_likelihood(mag - lc_model))
            
            else:
                pm.Normal("obs", mu=lc_model, sd=tt.exp(logs_lc), observed=mag)

    def init_orbit_OLD(self):
        """
